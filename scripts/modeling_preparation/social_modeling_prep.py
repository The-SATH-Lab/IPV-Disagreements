...
import pickle
from scripts.config import PROCESSED_SOCIAL_MESSAGING_DIR, PROCESSED_SOCIAL_EMA_DIR, SAVED_EMBEDDINGS_SOCIAL_DIR
from .helpers import calculate_average_response_time, clean_text, analyze_sentiment
import warnings
warnings.filterwarnings("ignore")

class SocialProcessor:
    def __init__(self, model_name='distilbert-base-uncased', batch_size=32, relationship='Romantic Partner', embedding_method='mean_pooling', save_dir='saved_embeddings'):
    ...
    def social_processing(self, dataframes):
        """Processes the SNAP messaging data."""
        participant_files = [df.columns[0] for df in dataframes]
        participant_ids = [int(file.split("_")[0]) if file.split("_")[0].isdigit() else int(file.split()[0]) for file in participant_files]
    ...
    def prepare_modeling_df(self, messaging_dir, ema_dir, embeddings=True, sentiment_analysis=False, message_count=False, return_train_test=False):
        ...
        social_dfs = pd.read_pickle(os.path.join(messaging_dir, "preprocessed_dfs.pkl"))
        ...
        filtered_dataframe = self.social_processing(social_dfs)
