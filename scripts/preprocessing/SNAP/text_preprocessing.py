...
from scripts.config import RAW_SNAP_MESSAGING_DIR, PROCESSED_SNAP_MESSAGING_DIR
import pickle
...
def preprocess_snap_messaging_data():
    new_columns = ['Message_Action', 'Text', 'Relationship', 'Application']
    dfs = []

    # Load and preprocess data
    for path in sorted(os.listdir(RAW_SNAP_MESSAGING_DIR)):
        if path.endswith(('.xlsx', '.csv')):
            df = load_data(os.path.join(RAW_SNAP_MESSAGING_DIR, path))
    ...
    # Save processed data
    target_file = os.path.join(PROCESSED_SNAP_MESSAGING_DIR, "preprocessed_dfs.pkl")
    with open(target_file, 'wb') as f:
        pickle.dump(dfs, f)

if __name__ == "__main__":
    preprocess_snap_messaging_data()