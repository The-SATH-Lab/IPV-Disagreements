...
def split_train_val_data(df, stratify_column='Disagreement', test_size=0.2, random_state=42):
    return train_test_split(df, test_size=test_size, random_state=random_state, stratify=df[stratify_column])

train_df, val_df = split_train_val_data(train_df)
...
def generate_test_prompt(text):
    return f"""
            Analyze the conversation between romantic partners provided after "Examples:".
            Determine whether there is a disagreement, and return "Yes" if there is a disagreement, 
            or "No" if there is not. Do not write anything else.
            ...
            Now, analyze the following conversation enclosed in square brackets.
            Determine whether there is a disagreement, and return "Yes" if there is a disagreement, 
            or "No" if there is not. Do not write anything else.
...
def apply_prompt_generation(df, generate_fn, few_shot_examples):
    return df.apply(lambda row: generate_fn(row['text'], row['Disagreement'], few_shot_examples), axis=1).reset_index(drop=True)

X_train = apply_prompt_generation(train_df, generate_prompt, few_shot_examples)
X_val = apply_prompt_generation(val_df, generate_prompt, few_shot_examples)
