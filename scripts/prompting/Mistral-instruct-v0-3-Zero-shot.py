...
# Suppress warnings and configure environment variables
...
def generate_test_prompt(text):
    return f"""
    [INST] Analyze the conversation between romantic partners enclosed in square brackets. 
    Determine if there is a disagreement, and return 'Yes' or 'No'. 
    Do not return anything else.[/INST]
...
X_train = train_df.apply(lambda row: generate_prompt(row['text'], row['Disagreement']), axis=1)
X_val = val_df.apply(lambda row: generate_prompt(row['text'], row['Disagreement']), axis=1)
X_test = test_df['text'].apply(generate_test_prompt)
...
X_test_social = social_df['text'].apply(generate_test_prompt)
